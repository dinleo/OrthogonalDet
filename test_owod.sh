...
if [ $BENCHMARK == "M-OWODB" ]; then
  ...
  python train_net.py --num-gpus 1 --dist-url tcp://127.0.0.1:${PORT} --task ${BENCHMARK}/t2_ft --config-file configs/${BENCHMARK}/t2_ft.yaml --eval-only MODEL.WEIGHTS output/${BENCHMARK}/t2_ft.pth

  python train_net.py --num-gpus 1 --dist-url tcp://127.0.0.1:${PORT} --task ${BENCHMARK}/t3_ft --config-file configs/${BENCHMARK}/t3_ft.yaml --eval-only MODEL.WEIGHTS output/${BENCHMARK}/t3_ft.pth

  python train_net.py --num-gpus 1 --dist-url tcp://127.0.0.1:${PORT} --task ${BENCHMARK}/t4_ft --config-file configs/${BENCHMARK}/t4_ft.yaml --eval-only MODEL.WEIGHTS output/${BENCHMARK}/t4_ft.pth
else
